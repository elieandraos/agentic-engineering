...
- Does the guide's closing material, however it's structured, leave the reader remembering the
  architectural abstraction itself — a contract, a pipeline, a boundary — rather than a list of
  features?
...
- Where the guide states a rule, guarantee, or boundary, is it grounded in a concrete enforcement
  reference — a path, symbol, schema element, config key, protocol, or runtime boundary — rather
  than asserted without evidence, without forcing a class/method shape onto a rule enforced some
  other way?
...
- If the guide covers reusable infrastructure alongside integration-specific or model-specific
  ...
  "specific" begins?
- Does the guide show what a new implementation or integration must *provide*, not everything it
  inherits for free? A section reading like a tutorial for the whole feature fails this question.
...
- Is lifecycle kept conceptually distinct from runtime ownership — a state machine and a
  responsibility handoff are different pictures even when they describe the same feature, and
  collapsing them into one diagram or one sentence shape is a finding?
...
- Are diagrams used for movement, ownership, or topology — not decoration? One that shows nothing
  moving or owned is a candidate for deletion, not a finding to soften.
...
- Is a volatile configuration value (a pagination size, a UI constant) presented as a durable
  architectural fact?
- Does the guide give a home — a layer, table, or diagram — to every feature it treats as core,
  such as one named in the opening or repeated across sections? A feature mentioned repeatedly
  without its ownership ever being pinned down is a finding, even if each mention is accurate on
  its own.
...
- Where the guide states future or deferred work, is it genuinely still open, with a stated
  ...
  finding.
- Are known limitations identified as intentional, with the reasoning stated, rather than reading
  as oversights? ("Deferred until a real one-hop model ships" is identified; a bare "TODO: handle
  this better" is not.)
...
- Can the reader reason safely about a future change from this guide alone — where responsibility
  lies, which guarantees matter, what would break?
