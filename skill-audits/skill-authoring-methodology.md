...
- **Evidence base:** the completed authoring, cohesion, and dossier work for `my-git-workflow` (`skill-audits/my-git-workflow.md`, `skill-audits/my-git-workflow-portability.md`) and `my-feature-planning` (`skill-audits/my-feature-planning.md`). Both skills share the same pre-authoring baseline commit, `fe5bd29` ("Externalize Phase A skills: my-feature-planning, my-git-workflow, my-architecture-laboratory"), which introduced every current `SKILL.md`, `README.md`, and `rules/*.md` file for both skills verbatim from their origin project. The authoring passes are the commits between `fe5bd29` and the current `main` HEAD (`d8cbaf293c544ce30e3bc042ac43dd6c9e8f13d7`).
- **Intended next validation:** use this methodology during another real skill-authoring pass before deciding what graduates into `skill-creator`.
...
- **Not a changelog:** this document is not a chronological change log and does not reproduce every historical edit. `skill-audits/my-git-workflow.md`, `skill-audits/my-git-workflow-portability.md`, and `skill-audits/my-feature-planning.md` own that history.
...
- Material product/architecture decisions and open implementation details are separate decisions. A useful test: would flipping the choice change what the system guarantees? If yes, it is material.
...
- Audits and dossiers preserve rationale, evidence, resolved history, and open questions.
...
Both completed passes needed a dedicated reconciliation step, not just per-rule rewrites. `my-feature-planning`'s cross-rule cohesion commit existed because `SKILL.md`, `README.md`, and one rule file still described sequencing, plan-input scope, and the handoff boundary the way they worked before the individual rule rewrites had already changed the underlying contract. `my-git-workflow`'s portability audit found the reverse failure mode: three leaked project-specific terms (a stack formatter name, a fused milestone-naming example, an ambiguous "this project" phrase) survived three prior audit iterations because each pass re-read the skill through the lens of whichever file it had just touched, rather than re-auditing the whole skill fresh. Together these show reconciliation is its own required pass — not an automatic byproduct of rewriting the owning file, and not safe to scope to only the file last touched.

## 14. Evidence-backed disposition register

Representative removed content from both completed passes, grouped by concept rather than by identifier.
...
|---|---|---|---|---|
| Framework artifact vocabulary (Laravel/PHP implementation-skill list in `SKILL.md`; "Pint"; `gatherMiddleware()`; Pest commands and `skipUnlessFortifyHas()` in `verification.md`; `useOrbit` design-system vocabulary in Track F) | Stack | Verification and formatting have a scope even when the specific tool is unnamed; a rule can name the integration category without naming the tool | Removed from portable prose; where kept, boxed under an explicit "what this repository's evidence shows" heading | Not decided here — a stack skill would need its own evidence that these are reusable across Laravel/Pest projects generally, not incidental to one project's setup |
| Origin-project domain names and issue/phase histories (`#288`/`#287`/`#120`/`#289`, PR `#298`, `v0.17.0`, "Phase 22 — Authentication & 2FA"; `useOrbit` model/column names) | Historical / project | The extraction happened; a worked example can teach message and evidence shape | Retained only where boxed as evidence (e.g. `commit-boundaries.md`'s four-issue table, `release.md`'s named release examples); removed everywhere fused into rule prose | No — this is origin-project history, not reusable knowledge |
...
| Project milestone naming and assignment defaults (`Phase NN — {Feature Name}`) | Project-specific, previously fused into a general definition | A milestone-completion test can be stated generally without requiring a specific naming syntax | The general test now stands alone; the naming example is illustrative only, not a required shape | No — one project's naming convention |
...
| Historical examples that accurately taught a surviving rule (`commit-boundaries.md`'s four-issue evidence table; `release.md`'s named release examples) | Historical evidence | The rule they teach is still current and correctly framed as a rule, not the example | Retained in the owning rule file, explicitly boxed as evidence, not as the rule itself | N/A — correctly placed already |
...
A `skill-creator` skill (Anthropic's official plugin) is installed locally and was inspected read-only for this document, without editing it. It is currently an eval/iteration-loop tool — capture intent, draft, write test prompts, run with-skill and baseline subagents, grade, benchmark, iterate on the triggering description, package — not an authoring-methodology document. It already uses progressive disclosure and already has a `references/` directory, which is the natural extension point below. It has no knowledge-classification model, no disposition ledger, and no controlled-technical-English guidance today; its own writing-style guidance favors persuasive, explanatory description text and explicitly warns against heavy-handed all-caps `MUST`/`NEVER` language. That guidance targets triggering descriptions and behavioral nudges, not normative rule-file prose, so it does not conflict with Section 12 — but it also does not already embody it, and this document should not claim otherwise.

The complete audit document — this file, and the two skill dossiers — should not be copied into `skill-creator`. After another real skill-authoring pass validates the method:
...
Candidate principles from this pass still needing another proving pass before graduation: the two-tier confidence model distinguishing rules grounded in repeated observation from rules grounded in one explicit human decision (Section 5); the eight-item "separate X from Y" conceptual-modeling checklist (Section 9), so far validated only within `my-feature-planning`; and the specific claim that concision has no useful lower bound at all, so far validated only across two skills' worth of evidence.
