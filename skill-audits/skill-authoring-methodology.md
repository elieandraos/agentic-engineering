...
- **Not a changelog:** this document is not a chronological change log and does not reproduce every historical edit. Per Section 10: Git history and historical audit evidence — including `skill-audits/my-git-workflow-portability.md` — preserve chronology, exact removed prose, and earlier repository states. Current dossiers — `skill-audits/my-git-workflow.md` and `skill-audits/my-feature-planning.md` — preserve current skill-specific architecture, rationale, and evidence-backed assessment. Runtime skill files hold the current operational contract. This document owns reusable authoring methodology and knowledge disposition.
