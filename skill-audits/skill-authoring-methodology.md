...
- **Evidence base:** the completed authoring, cohesion, and dossier work for `my-git-workflow` (`skill-audits/my-git-workflow.md`, plus the now-superseded portability audit preserved as historical evidence at `503e638bb926a7f437838cc0eb50f9ffbf59c766:skill-audits/my-git-workflow-portability.md`), `my-feature-planning` (`skill-audits/my-feature-planning.md`), `my-architecture-laboratory` (`skill-audits/my-architecture-laboratory.md`), and `my-laravel-stack` (`skill-audits/my-laravel-stack.md`). The first three skills share the same pre-authoring baseline commit, `fe5bd29` ("Externalize Phase A skills: my-feature-planning, my-git-workflow, my-architecture-laboratory"), which introduced every current `SKILL.md`, `README.md`, and `rules/*.md`/`references/*.md` file for all three skills. `my-git-workflow` and `my-feature-planning` were copied verbatim from their origin project at that commit; `my-architecture-laboratory` was externalized with its package-name, frontmatter, and path adaptation (`architecture-laboratory` renamed to `my-architecture-laboratory`, plus a `{{Project}}` placeholder fix) already applied, before its own authoring pass began. `my-git-workflow` and `my-feature-planning`'s authoring passes are the commits between `fe5bd29` and their shared authoring-evidence endpoint, `d8cbaf293c544ce30e3bc042ac43dd6c9e8f13d7`; `my-architecture-laboratory`'s is the twenty-commit pass between `fe5bd29` and its own endpoint, `b3c726ab263a719f7c179e342d150d42c0f457ee`. `my-laravel-stack` has a different starting point entirely: it was authored fresh from a gitignored origin-project skill via `56cddee47e9d84d6b244b41ec4536d24e8d7cff3:phase-d-stack-discovery.md`/`56cddee47e9d84d6b244b41ec4536d24e8d7cff3:phase-d-stack-synthesis.md`'s classification, not externalized from an existing `agentic-engineering` baseline; its eight-commit authoring pass runs from its creation at `ee72858` to its own endpoint, `4609cd50eb4f002036b5e1eb9e8a5c4ab01e7121`. Each skill's endpoint is its last pre-methodology-update authoring correction, not current `main` HEAD. This document was itself committed after each skill's respective endpoint.
- **Fourth proving pass completed:** `my-laravel-stack`'s authoring pass supplies a fourth distinct real proving pass within this authoring program (Sections 1, 9, 10, 13, and 16 integrate its evidence) — not independent-project validation. All four passes ran inside this same repository, and `my-laravel-stack`'s content specifically still derives from exactly one real consumer, `useOrbit` — the same one-consumer ceiling `56cddee47e9d84d6b244b41ec4536d24e8d7cff3:phase-d-stack-synthesis.md` itself states, not resolved by this document's integration of its evidence. Nothing here misrepresents this as independent-project validation. No further authoring pass is required before reassessing graduation; packaging the surviving principles into `skill-creator` itself remains the separate, later step Section 16 describes.
...
- verifying that a rule's stated conditionality survives every representation of it, not only its entrypoint statement — a content-block definition, table row, checklist item, or example can silently reimpose "mandatory" after the owning text already said "optional". `my-laravel-stack`'s resource-controller blueprint is a concrete instance: its later sections always allowed non-CRUD and single-action controllers that omit stages of the full composition, but its opening diagram still stated the complete Controller → Form Request → Policy → Action → Resource → Inertia chain with no qualifier, reading as a mandatory checklist despite the file's own later content already contradicting that reading. The fix was not new content — it was stating explicitly, at the point of first mention, that the full chain is a reference composition and each stage is conditional on the endpoint actually needing it. A caveat correctly stated once elsewhere in the same file does not make an earlier diagram, summary, or example conditional on its own — each representation must carry the same authority and optionality independently;
...
- A blueprint or composed shape may compose and route to the rules it draws on without restating their canonical matrices, tables, or decision content — describing what a rule owns is not the same act as owning it. A summary may name an owner; it must not silently become a second authority for the same decision.
...
Duplication is especially dangerous exactly where two files can each believe the other owns the decision, and so drift independently without either file's own text signaling a problem. `my-laravel-stack`'s authoring pass produced a concrete instance: its Pest testing blueprint originally restated the same per-layer test-ownership mapping and existence-vs-exact-value assertion rule that a separate, dedicated test-ownership rule already owned. The fix was not to reconcile the wording in both places — it was to delete the blueprint's copy entirely and leave the dedicated rule as the single canonical owner, with the blueprint routing to it by reference. This is the same principle as the `my-feature-planning`/`my-git-workflow` milestone-lifecycle routing above, now evidenced within a single skill's own internal files rather than only across a skill boundary.
...
- `README.md` explains the lifecycle and reasoning without becoming an operational duplicate. Do not add a README merely for symmetry with other skills that happen to have one. A README is justified specifically when a human maintainer needs a conceptual map, rationale, taxonomy, or lifecycle explanation that would otherwise burden or duplicate the always-loaded operational entrypoint — not as a default file every skill should carry. `my-laravel-stack` added `README.md` only once its authoring pass introduced a real taxonomy question (which resource kind — convention, composed shape, or installable implementation — a given file is) that the owner could not answer by opening `SKILL.md` alone; the README's job stayed the conceptual map, while `SKILL.md`'s routing table was left untouched rather than duplicated into prose. The two files must be read together during reconciliation to confirm they stay complementary, not restatements of each other — a README repeating a routing table, or a `SKILL.md` repeating a README's rationale, is a duplication defect even when both files are individually well-written.
...
**Semantic classification over historical filename or location.** A resource's type, directory, and name should follow how it changes agent decisions and how it is consumed — never its original location, its historical filename, or a desire for structural symmetry with sibling files. `my-laravel-stack`'s authoring pass is evidence, not a new universal directory contract: `filters-pattern.md` lived under `rules/` from its first authored commit, but its actual content — a Form Request plus concrete Filter/Sorter classes plus reusable base-class/trait templates plus controller wiring plus tests — was always a multi-component implementation shape, not an independently applicable convention; it was reclassified to `blueprints/filters-and-sorting.md` once that shape was recognized. `actions-pattern.md` stayed correctly classified as a rule throughout the same pass, yet its `-pattern` suffix wrongly implied blueprint semantics and was renamed to `actions.md` in a separate, later correction — evidence that a filename can mislabel a resource's kind even when the resource's actual classification was never wrong. Separately, that pass's PHP copy/adapt implementations were correctly classified as templates rather than prose rules or generator stubs (see the code-example/template standard below) — a third resource kind this same discipline must distinguish, not merely a rules-versus-blueprints question. The reusable principle:

> Choose a resource's type, location, and name from how it changes agent decisions and how it is consumed — not from its historical filename or a desire for structural symmetry.

This does not impose `rules/`, `blueprints/`, and `templates/` (or any other fixed taxonomy) as a universal directory structure every skill must adopt — it is one skill's evidence-backed answer for its own actual content, generalized only as the classification discipline behind it.

**Code examples and implementation templates.** Section 1 already treats supporting artifacts —
templates, examples, tables, checklists — as instruction-bearing and worthy of the same rigor as prose
rules. `my-laravel-stack`'s authoring pass generalizes that into a durable standard for two related but
distinct artifact kinds: illustrative code examples, and complete copy/adapt implementation templates.

For illustrative code examples, embedded in prose rules and blueprints:

- use neutral domain concepts rather than leaking the origin consumer's product vocabulary, models, or
  literals into a portable skill's public-facing examples;
- demonstrate one relevant contract at a time;
- preserve coherence across related snippets — when several examples form one flow, a reader following
  the whole chain should see one consistent example, not fragments from different fictional features;
- use one consistent established idiom for the same operation unless a contrast is intentional and
  explained, rather than silently mixing equivalent-but-different constructions for the same contract;
- do not present technically invalid or incomplete pseudocode as if it were runnable;
- illustrative snippets may omit irrelevant detail when the omission is clear, the way a concise example
  from high-quality framework documentation would.

`my-laravel-stack`'s own authoring evidence: its consistency pass standardized a Resource-construction
example (`new OrderResource(...)` in one file, the equivalent `OrderResource::make(...)` in another) to
one consistent idiom across every file illustrating the same contract — cited here only as evidence for
the general consistency principle, not as a Laravel rule this methodology adopts.

Copy/adapt implementation templates are held to a stricter bar than illustrative snippets, because a
reader is meant to install them directly rather than merely read them for understanding:
...
Call an artifact a generator stub only when an actual generator or placeholder-substitution mechanism
consumes it — never merely because it is a complete file meant to be copied. `my-laravel-stack` kept its
five reusable implementations as complete, syntactically valid source files (a single origin-stack's
choice of file extension) precisely because no generator or substitution contract exists for them; that
choice does not universalize any one file extension or artifact format to unrelated artifact types in
other skills — the criterion is whether a generator actually consumes the file, not what its extension
happens to be.
...
- When a resource moves between directories or is renamed, a bare filename reference can become ambiguous or point at the wrong file; identify the owning location unambiguously rather than relying on a filename alone once more than one directory could plausibly hold it. Validate every reference form a move can leave stale — an actual Markdown link (must resolve from the file that contains it), a path mentioned in code or a code comment, and a plain-prose filename mention all need checking, since a Markdown-link checker alone catches only the first kind. Treat a reclassification or move as requiring a repository-wide stale-reference search, not just an update at the reference nearest the change.

All four completed passes needed a dedicated reconciliation step, not just per-rule rewrites. `my-feature-planning`'s cross-rule cohesion commit existed because `SKILL.md`, `README.md`, and one rule file still described sequencing, plan-input scope, and the handoff boundary the way they worked before the individual rule rewrites had already changed the underlying contract. `my-git-workflow`'s now-deleted portability audit (historical evidence at `503e638bb926a7f437838cc0eb50f9ffbf59c766:skill-audits/my-git-workflow-portability.md`) found the reverse failure mode: three leaked project-specific terms (a stack formatter name, a fused milestone-naming example, an ambiguous "this project" phrase) survived three prior audit iterations because each pass re-read the skill through the lens of whichever file it had just touched, rather than re-auditing the whole skill fresh. `my-architecture-laboratory`'s pass adds a third failure mode neither predecessor exhibited: its corrected locked-vs-open materiality test lives in `my-architecture-laboratory/rules/plan-synthesis.md`, but a narrower restatement of that same test survived, uncorrected, inside `my-feature-planning/rules/plan-md-input.md` — a rule file in a different skill entirely, found only by reading that consumer directly rather than by re-reading the file that had just been fixed. `my-laravel-stack`'s pass adds a fourth, path-specific instance of the same underlying discipline: moving its two blueprint files into a new `blueprints/` directory and renaming `assets/app` to `templates/app` required updating every cross-reference across `SKILL.md`'s routing table, every blueprint-to-rule and rule-to-blueprint link, and every template installation path in the same commit — a partial move would have left some references resolving to a location the resource no longer occupied. Together these show reconciliation is its own required pass — not an automatic byproduct of rewriting the owning file, not safe to scope to only the file last touched, not safe to scope to only the skill being authored, and not safe to scope to only the reference nearest the moved resource.

## 14. Evidence-backed disposition register

Representative removed content from the first two completed passes (`my-git-workflow` and `my-feature-planning`), grouped by concept rather than by identifier. `my-architecture-laboratory`'s and `my-laravel-stack`'s skill-specific dispositions are retained instead in their own dossiers, not folded into this register.
...
The complete audit document — this file, and the four skill dossiers — should not be copied into `skill-creator`. Now that a fourth proving pass exists (see the status block above):
...
Candidate principles, reassessed individually now that a third pass (`my-architecture-laboratory`) and a fourth (`my-laravel-stack`) exist:

- The two-tier confidence model distinguishing rules grounded in repeated observation from rules grounded in one explicit human decision (Section 5) is not exercised by either the third or the fourth pass's evidence. Its status is unchanged — still resting on `my-git-workflow`'s single case — and it still needs its own proving pass before graduation.
- The "separate X from Y" conceptual-modeling checklist (Section 9) is strengthened, not graduated: `my-architecture-laboratory` independently produced its own instance of the pattern — two gates ("decisions approved before synthesis" and "the document approved after synthesis") collapsed into one at a document's point of first mention, found and corrected the same way Section 13's cross-skill example was. It is now supported by two skills' authoring evidence rather than one, but still short of the "more than one consuming project" bar Section 5 sets for the strongest claim, since both instances arose inside the same repository's authoring program. `my-laravel-stack`'s contribution to Section 9 is a related but distinct instance — duplicated ownership of one decision across two files, rather than two decisions collapsed into one — recorded there as its own evidence rather than folded into this same count.
- The claim that concision has no useful lower bound at all (Section 11) is strengthened: `my-architecture-laboratory`'s `rules/` files moved by different amounts and in different directions under the identical discipline — `doc-style.md` shrank by about 18%, `review.md` grew by about 6%, and `plan-synthesis.md` stayed within 2% of its baseline; its own `README.md`, held to the same discipline, shrank by about 24% — directly against a uniform target. Now supported by three skills' worth of evidence rather than two. `my-laravel-stack`'s pass is not folded into this specific word-count claim: its files moved by substantial and uneven amounts across the same authoring pass (for example `rules/actions.md` grew considerably across two correction commits while other files shrank or barely moved), but no comparable word-count measurement was taken for it, so it is left as directionally consistent evidence rather than a counted fourth data point for this claim.

**The semantic-classification, single-ownership-duplication, README-justification, and cross-directory-reference lessons `my-laravel-stack` contributes are integrated directly into Sections 9, 10, and 13 above, not listed as new Section 16 candidates here** — each is evidenced by real defects found and fixed during this pass, but each still rests on the same same-repository authoring program the rest of this section already discounts, and none has yet been forward-tested on a skill this program didn't itself author. Neither previously strengthened claim, nor these newly integrated ones, is declared universal by this reassessment; all remain working conclusions pending evidence from an actual, separate consuming project rather than another pass inside this same repository.
