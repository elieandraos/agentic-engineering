...
## Second resolution update (methodology correction pass)

A further round of direct human clarification corrected three specifics of the material the update above absorbed — none of these were open questions in the original analysis (§§1–12); they are corrections to how the prior absorption pass encoded newly-absorbed methodology, not resolutions of this review's own findings. Recorded here only, without altering the original analysis:

- **Backlog/hotfix branch readiness** (`rules/sequencing.md`) — the prior absorption wording ("work proceeds on whatever branch is already checked out — normally the trunk/main branch") understated the actual rule and left room for silently working on the wrong branch. Corrected to state directly that this work happens on the repository's trunk branch, with a mismatch between that and the actual checkout surfaced to the human rather than silently proceeded on or silently switched.
- **Post-merge milestone-closure/release ordering** (`rules/milestone-completion.md`, `rules/release.md`) — the prior absorption encoded release validation as a hard precondition of the closure gate ("starts only after release publication has been validated"), which does not match the observed workflow's typical order (closure before release drafting) and over-constrains the methodology. Corrected so both the closure gate and the release phase branch independently from the same post-merge authorization (`rules/release.md`'s step 0); neither rule's completion gates the other. The observed typical order on the workflow's home project (closure, then release) is now stated as an observed habit, not an enforced dependency in either direction. See `rules/milestone-completion.md`'s "Milestone closure and release do not gate each other."
- **Milestone-PR reference convention** (`rules/milestone-completion.md`) — previously recorded only as contextual observation ("this rule records that as context, not as something it checks or enforces"). Promoted to an explicitly stated observed contract ("A PR carrying milestone work is expected to reference the milestone it integrates") while keeping PR creation/content entirely unenforced and human-owned. See `rules/milestone-completion.md`'s "The milestone-PR reference convention."

The dependency-order vs. activation-order question (§4/§12) remains explicitly unresolved — no real conflict has been observed, and this pass found no new evidence bearing on it.
