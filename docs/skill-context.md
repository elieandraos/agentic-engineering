...
subtotal alone understates this row's own modeled workflow total, not merely its ceiling. Every
figure in this section is a directly measured file size or a modeled sum built from such measurements
plus a rough token approximation (characters ÷ 4) — never an observed session's actual consumption.
...
| **CI-failure investigation and correction handoff** | `SKILL.md` (6,391) + `rules/ci-failure-correction.md` (8,937) = 15,328 chars, 3,832 tokens | *Mandatory, not merely conditional.* + `implement-it/rules/review-gates.md` (13,016, cross-skill) = 28,344 chars, 7,086 tokens. Step 3 of this procedure unconditionally directs consulting that file's "when to stop and ask" standard to determine whether a correction stays in scope — every real investigation reaches it, not only some. The local-file subtotal alone therefore does not model this path's complete file-loading total; 28,344 chars / 7,086 tokens is the closer modeled figure, not a measured or observed one. (This file's own citations of `rules/milestone-completion.md` remain orientation only, confirmed unchanged from the prior pass — naming what it is and where it starts, not an instruction to consult its content.) Two further handoffs stay outside this row's subtotal entirely, both conditional on what step 3 finds: step 5's routing to `plan-it/rules/discovered-work.md` (10,783 chars per the file-size table above) when the finding is genuinely unscoped work, and step 6's handoff to `implement-it`'s own lifecycle once a fix is authorized — Gate 1/Gate 2, `commit-boundaries.md`, `verification.md`. Neither is added to this row's own totals. The `implement-it` handoff can be roughly approximated from this document's own "implement-it, one ordinary issue" workflow row above, since a delivery correction runs the same lifecycle; the `plan-it` discovered-work intake has no modeled workflow row in this document at all — only its own file size is listed |
