...
```
implement + verify → Review implementation → derive commit plan → Commit plan → build commits
```

**Review implementation** and **Commit plan** are the human-facing names of the two pre-merge
approvals. `Gate 1` and `Gate 2` remain internal lifecycle identifiers when precise rule references
need them. The two approvals are never collapsed, and approval at Review implementation does not
imply approval at Commit plan. No commit is created before Commit plan is approved.
...
is not part of either approval below — a merged PR does not satisfy it.
...
The final line should state the exact decision requested and, when the next step is already authorized
by the current state, name what approval unlocks. For example:

> **Review implementation:** approve to proceed to Commit plan.

Then wait for explicit human approval. Approval here authorizes moving on to commit planning — nothing
more. Do not begin deriving commit structure before it.
...
- **An authorized delivery correction.** No approved issue is required to reach this approval at all
...
Either way, this approval always supplies whatever intended-scope evidence it actually has to
`review-it` — it never invokes `review-it` with no scope evidence at all, unlike a standalone caller
who may genuinely have none. Treat `review-it`'s result as follows before reporting here:

- **Clean.** Report it as such and proceed to the Review implementation report above.
- **Findings, within this skill's authorized scope to resolve.** Fix them, then request a re-review
  ...
  their resolution, plus the re-review's clean result, at Review implementation.
- **A finding that reveals a genuine unresolved decision** — architecture, scope, or a choice with
  no clearly better answer — is not this approval's to resolve silently. Stop and ask, per "When to
...
- **A material limitation `review-it` reports** — missing evidence, an unreachable diagnostic, or an
  ambiguity it proceeded past rather than resolved (`review-it/rules/scope.md`'s report shape). When
  the missing evidence is actually obtainable from this skill's own context (the approved issue, the
  authorized correction's scope, a project instruction `review-it` couldn't reach), supply it and
  request a fresh pass. When it isn't obtainable, report the specific blocker and the human decision
  it requires — this is a stop under "When to stop and ask," below, not a silent gap. An optional
  absent artifact that `review-it` itself treated as inapplicable — no stack companion, no PR
  description beyond what was already supplied — does not by itself trigger this bullet; only a
  limitation that leaves a required check genuinely unresolved does.
...
not, by itself, the Review implementation approval — it is the same kind of pause "When to stop and
ask" already describes, and the approval still requires its own explicit human decision of the
complete report afterward.

## Commit plan — second approval (Gate 2)

Only after Review implementation is approved:
...
Get explicit human approval of the complete plan before writing a single commit.

The final line should state the exact decision requested. For example:

> **Commit plan:** approve to create the proposed commit(s).

Approval here authorizes creating commits and nothing else. It does not authorize push or issue
closure.
...
## Approval validity before Commit plan and before push
...
Before Commit plan, and again before requesting push authorization (`rules/push-readiness.md`), check
that the current work, its scope, and the action about to be proposed still match what the relevant
approval actually covered:

- **A material change requires the affected review and approval to be renewed.** A scope change
  since Review implementation, a diff that no longer matches what Commit plan approved, or an issue
  body edited since its own approval each invalidate the approval that covered the prior state —
  re-review and re-approve the affected surface before relying on it again. This is the same principle
...
- **Preserve an approval that demonstrably remains applicable.** This check exists to catch a real
  ...
  content into its already-approved commits must not automatically invalidate Review implementation
  merely because `HEAD` moved or the remaining working-tree diff changed shape while commits were
  being built — that's the expected, unavoidable effect of committing, not evidence the approved
  content changed. Confirm the actual content is unchanged before treating an approval as still valid,
  and confirm it again before treating it as stale.
- **Remote commit reachability proves presence, not verification or authorization.** Finding a
  ...
  there — it is not evidence that Review implementation, Commit plan, or push authorization actually
...
into an implementation fact by silently choosing one. Stop, at either approval or during
...
**Do**
- Stop at Review implementation once implementation, verification, and a clean-or-resolved `review-it`
  pass are complete.
- Include the `Activated skills:` line in the Review implementation report, restating the activation checkpoint.
- Invoke `review-it` before reporting at Review implementation, and again, scoped to the affected surface,
  after fixing any finding it raises.
- End each human approval report with the exact decision requested and, when the workflow already makes
  the next action deterministic, the action that approval will unlock.
- Derive the Commit plan only after Review implementation is approved.
- Show the complete Commit plan before creating any commit, and get explicit approval of it.
- Check, before Commit plan and before requesting push authorization, that the current work, scope, and
  proposed action still match what was actually approved — renewing the affected approval on a material
  change, and preserving one that demonstrably still applies.
- Investigate a genuine unknown and offer a recommendation before asking the human to decide.

**Don't**
- Collapse Review implementation and Commit plan into one approval.
- Treat implementation approval as Commit plan approval.
- Invalidate Review implementation merely because `HEAD` or the remaining diff changed while assembling
  unchanged, already-approved content into its already-approved commits.
- Treat a commit's mere presence on the remote branch as evidence it was ever reviewed or authorized.
- Report a `review-it` finding as resolved without an actual fix and an actual re-review behind that claim.
- Report `review-it`'s material limitation as a clean result, or leave it unaddressed when the missing
  evidence was actually obtainable.
- Treat asking the human about a `review-it` finding or limitation as Review implementation approval.
- Treat a clean `review-it` result as authorization by itself — it is evidence Review implementation's
  report cites, not a substitute for the human's approval.
- Create a commit before Commit plan is approved.
