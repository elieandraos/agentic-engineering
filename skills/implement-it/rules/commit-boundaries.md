...
4. **Isolate the correction's own hunk(s) explicitly, from every difference the path shows against
   ...
   *additional* unrelated hunk as part of the correction the moment more than one thing differs from
   `head` for the same path — reproduced concretely (three separate hunks in one file: one unrelated
   hunk already staged, one unrelated hunk still unstaged, and the correction itself unstaged; the
   prior extraction-by-subtraction folded the unstaged unrelated hunk into the "correction" and
   round-tripped anyway — see `scenarios.md`). Never derive the correction by subtracting a
   known piece from `combined` — establish it positively instead:
   ...
   - **If the correction can't be cleanly separated this way, stop** — this is a capture-time
     ...
     - one hunk mixes the correction with something else, with no way to select one without the
       other (`git add -p`'s own split may refuse outright — reproduced concretely on a single-line
       change adjacent to a single-line unrelated insertion of identical text; see `scenarios.md`);
...
5. **Extract the unrelated-only content, and verify the split by round-trip.** A successful command
   ...
   conflict (reproduced concretely; see `scenarios.md`). Use a real three-way merge instead — and
   because the correction is already known, positively, from step 4, this step only needs to compute
   its complement, never guess at the correction itself:
   ```
...
6. **Before clearing any real path, stashing, or resetting, confirm the unrelated content's
   ...
   - **otherwise** — `indexblob` matches neither. Either the real index already contains a mix of
     the correction and unrelated content for this path (reproduced concretely: both staged together,
     nothing further unstaged; see `scenarios.md`), or the unrelated content itself spans a staged
...
7. **Exclude a correction-touched shared path from the general protection step below; protect
   ...
   merge handles cleanly (verified in practice; see `scenarios.md`).
