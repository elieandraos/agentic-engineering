...
current boundaries and confidence. [`SKILL.md`](../skills/ship-it/SKILL.md) remains the operational
routing entrypoint; [`README.md`](../skills/ship-it/README.md) is the human-facing walkthrough. This
document explains the lifecycle architecture behind both rather than restating either.

## 1. Purpose and entry boundary

`ship-it` begins only from already-approved implementation scope — normally one approved
GitHub issue, or an approved set of them inside a milestone, produced by `plan-it`. It
...
milestone intended to ship as a release (a classification `plan-it` makes, not this
skill):
...
**Upstream handoff, from `plan-it`.** This skill's only input is already-approved work:
...
failure that reveals real, unscoped work routes to `plan-it`'s Discovered-work intake,
...
have already closed is a case neither this skill nor `plan-it` currently owns undoing,
