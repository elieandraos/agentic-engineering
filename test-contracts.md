...
| COMMON-10 | discovers a deferred predecessor decision before implementing its dependent issue | A closed predecessor issue's state or comments intentionally defer a dependent behavior to a specific later issue; a fresh session implementing that later issue inspects current repository/GitHub evidence, locates the applicable deferred decision, and reconciles it against the current issue's approved scope before implementing — the comment is evidence, not automatic scope authorization. Check that discovery occurs when the decision is actually relevant, not that every historical issue comment is read. | — |
