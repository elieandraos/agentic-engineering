...
| COMMON-11 | consumes propagated future context in a fresh implementation session | A dependent issue contains concise context propagated from a completed issue. In a fresh session with no prior conversation context, the implementation worker reads the issue/comment context, explicitly acknowledges the relevant decision before the first implementation write, and applies it. The evidence must distinguish actual context retrieval and acknowledgment from merely matching the final implementation. | implement-it issue-closure, verified useOrbit #326 → #327 |
